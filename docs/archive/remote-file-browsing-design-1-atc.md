...
# Tech Design — Remote File Browsing, Part 1: atc `/api/fs`
...
> `atc` repo (https://github.com/jeremytondo/atc), so the atc
...
> handed to a session working only in the atc repo.

## 1. Scope

Add a read-only remote filesystem browse API to atc:
...
- POSIX-only assumptions (path separators, dot-hidden convention) are fine —
  atc targets Linux/macOS workstations.
