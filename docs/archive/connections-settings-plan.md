...
AtelierCode should support multiple named Connections to Cockpit servers while keeping Projects and Terminal Sessions owned by the Cockpit API. The app should show Projects from all configured Connections in one project-first sidebar, make the Connection visible on each Project row, and provide a robust macOS Settings window for Connection management.

## Decisions

- A Connection is local to AtelierCode. It has an app-chosen name, API URL, optional token, stable local identity, and creation-order position.
- Projects remain Cockpit-owned records. AtelierCode displays each Project through the Connection it came from.
- Terminal Sessions remain Cockpit-owned records created on a particular Cockpit server. AtelierCode controls them through their Connection.
...
- AtelierCode v1 only surfaces project-scoped Terminal Sessions in the app UI. Cockpit API/CLI can continue to support unscoped Terminal Sessions.
...
- Deleting a Connection is local-only. It never deletes Projects or Terminal Sessions from Cockpit. Delete always confirms and explains that server data remains.
- Project archive visibility is a project-list filter control, not an app Settings item.
- Projects with active Terminal Sessions cannot be archived. Active means `starting` or `running`; this should be enforced by Cockpit and mirrored in AtelierCode when possible.

## Non-Goals

- No Cockpit API changes for Connection storage.
...
4. Update the project sidebar.
   ...
   - Remove `Other Sessions` from the AtelierCode UI.
