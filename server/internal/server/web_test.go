...
func TestWebAssetRoutesServesIndex(t *testing.T) {
	...
	webAssetRoutes(fstest.MapFS{
		"index.html": {Data: []byte("<h1>atc is running</h1>")},
	}).ServeHTTP(rec, req)

	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want %d", rec.Code, http.StatusOK)
	}
	if !strings.Contains(rec.Body.String(), "atc is running") {
		t.Fatalf("body = %q, want atc page", rec.Body.String())
	}
}
...
func TestWebAssetRoutesServesStaticAsset(t *testing.T) {
	...
	webAssetRoutes(fstest.MapFS{
		"index.html":             {Data: []byte("<h1>atc</h1>")},
...
func TestWebAssetRoutesFallsBackToShellForBrowserRoutes(t *testing.T) {
	...
	webAssetRoutes(fstest.MapFS{
		"index.html":    {Data: []byte("<h1>atc index</h1>")},
		"fallback.html": {Data: []byte("<h1>atc shell</h1>")},
	}).ServeHTTP(rec, req)

	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want %d", rec.Code, http.StatusOK)
	}
	if !strings.Contains(rec.Body.String(), "atc shell") {
...
func TestWebAssetRoutesDoesNotServeHiddenSentinels(t *testing.T) {
	...
	webAssetRoutes(fstest.MapFS{
		"index.html":      {Data: []byte("<h1>atc shell</h1>")},
