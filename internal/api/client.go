...
// request path sets the bearer token, Atc-Client-Version, and
// Atc-Protocol, reads their server counterparts on every exchange, and
// refuses a server on another protocol, so the compatibility contract
// lives in exactly one place; each operation is a small typed method
// over it. The server's release rides Health and every Problem, for
...
// tokenless probing — the server's version and protocol still come back
// on the typed error of the resulting 401. version is the client build
// identity, for diagnostics. A nil httpClient defaults to a plain
// http.Client; pass one to set timeouts or transports.
...
// It rides the same request path as every typed method (auth, identity
// headers, protocol check) but leaves status handling to the caller.
...
func (c *Client) send(ctx context.Context, method, path string, body io.Reader, contentType string) (*http.Response, error) {
	...
	// The client's half of the protocol contract (ATC-325): whatever the
	// status, an answer from an ATC server on another protocol — or from
	// one that predates the contract and sent none — is refused before
	// any body is read, and the typed error carries what answered. An
	// answer that is not from an ATC server at all (a proxy's error page
	// while the backend is down) keeps its own status instead.
	serverProtocol := responseProtocol(resp)
	success := resp.StatusCode >= http.StatusOK && resp.StatusCode < http.StatusMultipleChoices
	atcAnswered := serverProtocol != 0 || resp.Header.Get(ServerVersionHeader) != "" || success
	if serverProtocol != Protocol && atcAnswered {
		_, _ = io.Copy(io.Discard, io.LimitReader(resp.Body, maxBodyRead))
		_ = resp.Body.Close()
		return nil, protocolMismatch(resp, serverProtocol)
	}
	return resp, nil
}

// responseProtocol reads the server's protocol off a response; 0 when the
// header is absent or unreadable.
func responseProtocol(resp *http.Response) int {
	protocol, err := strconv.Atoi(resp.Header.Get(ProtocolHeader))
...
// protocolMismatch is the client-synthesized refusal of an incompatible
// server, in the same shape the server uses for an incompatible client.
func protocolMismatch(resp *http.Response, serverProtocol int) *Problem {
	detail := fmt.Sprintf("the server sent no ATC protocol (is it an ATC server?); this client speaks protocol %d", Protocol)
	if serverProtocol != 0 {
		detail = fmt.Sprintf("the server speaks ATC protocol %d; this client speaks protocol %d", serverProtocol, Protocol)
	}
	return &Problem{
...
func problemFrom(resp *http.Response) *Problem {
	...
	problem.ServerProtocol = responseProtocol(resp)
	return problem
}
