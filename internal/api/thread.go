...
type Thread struct {
	...
	PendingTurn    *PendingTurn `json:"pendingTurn,omitempty" doc:"A turn ATC submitted (a thread create) that the provider has not reported starting yet; omitted when none. Its id becomes latestTurn's once the provider starts the turn."`
