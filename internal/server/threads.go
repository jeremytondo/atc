...
func registerThreads(humaAPI huma.API, service *threads.Service, coordinator *application.Coordinator) {
	...
	huma.Register(humaAPI, huma.Operation{
		...
		Description:   "Answers a request the thread's agent is blocked on (the thread's inputRequests): with structured answers — each question at most once, with the choices it offers or a custom text where it allows one; a subset of the questions is accepted, since the provider forwards what is answered and the agent reads what it received — or with a reply, the user's text passed to the agent verbatim to interpret against its questions (an answer in prose, a partial answer, a change of direction). Returns 202 with the request carrying the answer: delivery is accepted once the program committed it, uncertain when it never answered (submit the same answer again to reconcile); state stays sent until the provider's evidence resolves the request — with exactly this answer, and the request reads resolved by answer — or reports a failure. The request is never resolved on the program's acceptance alone, and never on its disappearance. Resubmitting the same answer recovers the recorded one; a different answer is refused while it awaits evidence. Refusals: 400 for an answer that does not answer the request (nothing answered, an unknown question, a choice not offered, a form not allowed, a reply together with answers), a request ATC cannot answer (its unanswerable reason), or an Integration that cannot answer; 404 for an unknown thread or request; 409 for a request already resolved, a different answer still awaiting evidence, or a stop being confirmed on the thread; 503 while the Integration is not connected; 502 when the program rejects the answer (the request takes another).",
		DefaultStatus: http.StatusAccepted,
	}, func(ctx context.Context, input *struct {
		ID        string `path:"id" doc:"Thread identifier."`
		RequestID string `path:"requestId" doc:"Input request identifier."`
