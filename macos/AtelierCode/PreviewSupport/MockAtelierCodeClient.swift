...
extension AppModel {
    ...
    static func preview(connections: [(name: String, client: any AtelierCodeClient)]) -> AppModel {
        ...
        return AppModel(connections: store, clientFactory: { clientsByID[$0.id] ?? MockAtelierCodeClient() })
    }
}

/// Canned-data client for previews and offline development.
nonisolated struct MockAtelierCodeClient: AtelierCodeClient {
    ...
    var mockSessions: [Session] = [
        Session(
            ...
            updatedAt: Date(timeIntervalSinceNow: -60),
            project: MockAtelierCodeClient.atelierRef
        ),
        Session(
            id: "ses_starting",
            action: "codex",
            ...
            project: MockAtelierCodeClient.blazerrRef
        ),
        Session(
            id: "ses_failed",
            action: "lazygit",
    ...
    func session(id: String) async throws -> SessionDetail {
        guard let session = mockSessions.first(where: { $0.id == id }) else {
            throw AtelierCodeError.api(code: "session_not_found", message: "session not found: \(id)", sessionID: id)
        }
    ...
    func archiveSession(id: String) async throws -> SessionDetail {
        ...
        guard session.status == .terminated || session.status == .failed else {
            throw AtelierCodeError.api(code: "session_live", message: "session is still running", sessionID: id)
        }
    ...
    func action(name: String) async throws -> AtelierCodeAction {
        try actionRegistry.detail(name: name)
    }

    func createAction(_ request: ActionWriteRequest) async throws -> AtelierCodeAction {
        try actionRegistry.create(request)
    }

    func updateAction(name: String, _ request: ActionWriteRequest) async throws -> AtelierCodeAction {
        try actionRegistry.update(name: name, request)
    }

    func setActionEnabled(name: String, enabled: Bool) async throws -> AtelierCodeAction {
    ...
    func environments() async throws -> [AtelierCodeEnvironment] {
        ...
        """#.utf8)
        struct Envelope: Decodable { var environments: [AtelierCodeEnvironment] }
    ...
    func listDirectory(path: String, showHidden: Bool) async throws -> DirectoryListing {
        ...
        guard let children = mockTree[resolvedPath] else {
            throw AtelierCodeError.api(code: "not_found", message: "not found: \(resolvedPath)", sessionID: nil)
        }
...
nonisolated final class MockActionRegistry: @unchecked Sendable {
    ...
    private var fileEntries: [String: AtelierCodeAction] = [
        "lazygit": AtelierCodeAction(
    ...
    func detail(name: String) throws -> AtelierCodeAction {
        try lock.withLock { try requireEffective(name: name) }
    }

    func create(_ request: ActionWriteRequest) throws -> AtelierCodeAction {
        try lock.withLock {
    ...
    func update(name: String, _ request: ActionWriteRequest) throws -> AtelierCodeAction {
        try lock.withLock {
    ...
    func setEnabled(name: String, enabled: Bool) throws -> AtelierCodeAction {
        try lock.withLock {
    ...
    private func effective(name: String) -> AtelierCodeAction? {
    ...
    private func requireEffective(name: String) throws -> AtelierCodeAction {
    ...
    private func origin(of entry: AtelierCodeAction, name: String) -> String {
        guard let builtin = builtins[name] else { return "custom" }
        return sameExceptEnabled(entry, builtin) ? "builtin" : "modified"
    }

    private func sameExceptEnabled(_ a: AtelierCodeAction, _ b: AtelierCodeAction) -> Bool {
    ...
    private func store(_ action: AtelierCodeAction, name: String) {
    ...
    private func definition(from request: ActionWriteRequest, name: String) -> AtelierCodeAction {
        AtelierCodeAction(
            name: name,
            origin: "custom",
