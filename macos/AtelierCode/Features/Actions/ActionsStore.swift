...
final class ActionsStore {
    ...
    private(set) var actions: [AtelierCodeAction] = []
    ...
    func detail(name: String) async throws -> AtelierCodeAction {
        try await client.action(name: name)
    }

    /// Mutations throw so the editor can show inline errors.
    @discardableResult
    func create(_ request: ActionWriteRequest) async throws -> AtelierCodeAction {
    ...
    @discardableResult
    func update(name: String, _ request: ActionWriteRequest) async throws -> AtelierCodeAction {
    ...
    @discardableResult
    func setEnabled(name: String, enabled: Bool) async throws -> AtelierCodeAction {
    ...
    func action(name: String) -> AtelierCodeAction? {
        actions.first { $0.name == name }
    }

    private func merge(_ action: AtelierCodeAction) {
    ...
    private static func sorted(_ actions: [AtelierCodeAction]) -> [AtelierCodeAction] {
        actions.sorted {
