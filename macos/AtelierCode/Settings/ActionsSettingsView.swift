...
/// The Actions settings section. Actions live on the Atelier Code server, so the
...
struct ActionsSettingsView: View {
    ...
    private var selectedAction: AtelierCodeAction? {
    ...
    var body: some View {
        if appModel.runtimes.isEmpty {
            ContentUnavailableView {
                ...
                Text("Actions live on an Atelier Code server. Add a connection in the Connections tab, then manage its actions here.")
            }
        } else {
            VStack(spacing: 0) {
                header
            ...
            .confirmationDialog(removePrompt, isPresented: $confirmRemove) {
                ...
                Text(
                    selectedAction?.isModified == true
                        ...
                        : "This removes the action from the Atelier Code server for every client."
                )
            }
        }
    }
    ...
    private func enabledBinding(for action: AtelierCodeAction) -> Binding<Bool> {
        Binding(
...
private struct ActionEditorView: View {
    ...
    @State private var original: AtelierCodeAction?
...
#Preview("Editor — codex (params)") {
    ActionEditorView(
        store: ActionsStore(client: MockAtelierCodeClient()),
