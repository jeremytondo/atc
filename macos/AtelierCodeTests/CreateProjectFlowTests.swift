...
private nonisolated final class RecordingClient: AtelierCodeClient, @unchecked Sendable {
    private let inner = MockAtelierCodeClient()
    ...
    func actions() async throws -> [AtelierCodeAction] { try await inner.actions() }
    func action(name: String) async throws -> AtelierCodeAction { try await inner.action(name: name) }
    ...
    func environments() async throws -> [AtelierCodeEnvironment] { try await inner.environments() }
...
struct CreateProjectFlowTests {
    ...
    func preselectionUsesFirstRuntime() {
        let model = AppModel.preview(connections: [
            (name: "A", client: MockAtelierCodeClient()),
            (name: "B", client: MockAtelierCodeClient()),
        ])
    ...
    func reselectingSameConnectionKeepsFolder() {
        let model = AppModel.preview(connections: [(name: "A", client: MockAtelierCodeClient())])
        let draft = CreateProjectDraft()
        draft.preselectFirst(in: model.runtimes)
        draft.workingDir = "/home/dev/Projects/atelier"
