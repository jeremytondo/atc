...
/// `HTTPAtelierCodeClient`'s real query construction can be asserted.
...
struct FileSystemRequestTests {
    private func makeClient() -> HTTPAtelierCodeClient {
        let configuration = URLSessionConfiguration.ephemeral
        configuration.protocolClasses = [CapturingURLProtocol.self]
        let server = AtelierCodeServer(baseURL: URL(string: "http://example.com:7331")!)
        return HTTPAtelierCodeClient(server: server, session: URLSession(configuration: configuration))
    }
