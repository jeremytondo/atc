...
@Suite("AtelierCodeServer URL building")
struct AtelierCodeServerTests {
    let server = AtelierCodeServer(baseURL: URL(string: "http://workstation.tail1f9a09.ts.net:7331")!)
    ...
    func attachURL() {
        ...
        let tls = AtelierCodeServer(baseURL: URL(string: "https://example.com")!)
    ...
    func authHeaders() {
        ...
        let withToken = AtelierCodeServer(baseURL: server.baseURL, token: "sekrit")
        #expect(withToken.authHeaders == ["Authorization": "Bearer sekrit"])
        let emptyToken = AtelierCodeServer(baseURL: server.baseURL, token: "")
