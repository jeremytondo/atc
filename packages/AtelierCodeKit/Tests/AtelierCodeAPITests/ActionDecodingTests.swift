...
struct ActionDecodingTests {
    ...
    func actions() throws {
        let envelope = try JSONDecoder.atelierCode().decode(ActionsEnvelope.self, from: actionsFixture)
    ...
    func environments() throws {
        let envelope = try JSONDecoder.atelierCode().decode(EnvironmentsEnvelope.self, from: environmentsFixture)
    ...
    func actionDetail() throws {
        ...
        """#.utf8)
        let detail = try JSONDecoder.atelierCode().decode(AtelierCodeAction.self, from: detailFixture)
        #expect(detail.command == "claude")
        #expect(detail.args == ["--dangerously-skip-permissions"])
        #expect(detail.isModified)

        let listEntry = try JSONDecoder.atelierCode()
