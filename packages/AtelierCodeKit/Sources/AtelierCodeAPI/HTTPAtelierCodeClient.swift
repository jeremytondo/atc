...
/// URLSession-backed implementation of `AtelierCodeClient`.
public struct HTTPAtelierCodeClient: AtelierCodeClient {
    public let server: AtelierCodeServer
    private let session: URLSession
    private let decoder: JSONDecoder

    public init(server: AtelierCodeServer, session: URLSession = .shared) {
        self.server = server
        self.session = session
        self.decoder = .atelierCode()
    }

    // MARK: - AtelierCodeClient
    ...
    public func actions() async throws -> [AtelierCodeAction] {
        let envelope: ActionsEnvelope = try await get("actions")
        return envelope.actions
    }

    public func action(name: String) async throws -> AtelierCodeAction {
        try await get("actions/\(name)")
    }

    public func createAction(_ request: ActionWriteRequest) async throws -> AtelierCodeAction {
        try await post("actions", body: request)
    }

    public func updateAction(name: String, _ request: ActionWriteRequest) async throws -> AtelierCodeAction {
        try await put("actions/\(name)", body: request)
    }

    public func setActionEnabled(name: String, enabled: Bool) async throws -> AtelierCodeAction {
    ...
    public func environments() async throws -> [AtelierCodeEnvironment] {
    ...
    private func decode<T: Decodable>(_ data: Data) throws -> T {
        do {
            ...
            throw AtelierCodeError.decoding(underlying: error)
        }
    }
    ...
    private func send(
        ...
        do {
            ...
            throw AtelierCodeError.transport(underlying: error)
        }

        guard let http = response as? HTTPURLResponse else {
            throw AtelierCodeError.badStatus(-1)
        }
        guard (200..<300).contains(http.statusCode) else {
            ...
            throw AtelierCodeError.badStatus(http.statusCode)
        }
        return data
    }
}
