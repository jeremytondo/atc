// A pending provider request (question or approval) as answerable content.
// Rendered in two homes with different chrome: inline on the transcript row
// of the item it blocks (`InlineRequestCard`), and in the bottom bar's
// glass card for requests without an item (the macOS shell owns that
// wrapper). Answers go to the caller; the row/card disappears when the
...
public struct ChatRequestContentView: View {
    let request: ThreadRequest
    let answer: (ThreadRequestAnswer) -> Void

    public init(request: ThreadRequest, answer: @escaping (ThreadRequestAnswer) -> Void) {
