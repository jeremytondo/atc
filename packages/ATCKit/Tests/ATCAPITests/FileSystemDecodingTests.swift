...
struct FileSystemDecodingTests {
    ...
    func listDecodes() throws {
        let listing = try JSONDecoder.atc().decode(DirectoryListing.self, from: listFixture)
    ...
    func emptyDecodes() throws {
        let listing = try JSONDecoder.atc().decode(DirectoryListing.self, from: emptyListFixture)
    ...
    func truncatedDecodes() throws {
        let listing = try JSONDecoder.atc().decode(DirectoryListing.self, from: truncatedListFixture)
        #expect(listing.truncated)
    }
    ...
    func forwardCompatibleKind() throws {
        ...
        """#.utf8)
        let listing = try JSONDecoder.atc().decode(DirectoryListing.self, from: body)
...
struct FileSystemErrorTests {
    ...
    func decodes(code: String) throws {
        ...
        let error = ATCError.api(code: envelope.error, message: envelope.message, sessionID: envelope.sessionId)
