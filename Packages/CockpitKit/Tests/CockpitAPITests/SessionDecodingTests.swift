...
struct SessionDecodingTests {
    ...
    func listDecodes() throws {
        let envelope = try JSONDecoder.cockpit().decode(SessionsEnvelope.self, from: sessionsFixture)
        ...
        #expect(named.displayName == "Cockpit Claude Test")
    ...
    func detailDecodes() throws {
        let detail = try JSONDecoder.cockpit().decode(SessionDetail.self, from: detailFixture)
