...
private nonisolated final class ScriptableClient: CockpitClient, @unchecked Sendable {
    ...
    private func gate() async throws {
        ...
        if shouldFail {
            throw CockpitError.api(code: "unreachable", message: "scripted failure", sessionID: nil)
        }
    }
    ...
    func actions() async throws -> [CockpitAction] { try await gate(); return try await inner.actions() }
